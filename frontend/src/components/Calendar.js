...
const Calendar = ({ chatUserId, compact = false }) => {
  ...
  const handleUpdateStatus = async (status) => {
    try {
      ...
      // Validate selectedMeeting exists
      if (!selectedMeeting || !selectedMeeting._id) {
        setError('No meeting selected for status update');
        return;
      }
      ...
      if (err.response?.status === 404) {
        setError('Meeting not found. It may have been deleted.');
        ...
        setError(err.response?.data?.error || `Failed to ${status} meeting`);
      }
    }
  };
  ...
  const handleEditMeeting = () => {
    // Validate selectedMeeting exists
    if (!selectedMeeting || !selectedMeeting._id) {
      setError('No meeting selected for editing');
      return;
    }
  ...
  const handleUpdateMeeting = async () => {
    try {
      ...
      // Validate selectedMeeting exists
      if (!selectedMeeting || !selectedMeeting._id) {
        setError('No meeting selected for update');
        return;
      }
  ...
  const handleDeleteMeeting = async () => {
    try {
      ...
      // Validate selectedMeeting exists
      if (!selectedMeeting || !selectedMeeting._id) {
        setError('No meeting selected for deletion');
        return;
      }
      ...
      if (err.response?.status === 404) {
        // Meeting already deleted, just remove from local state
        setMeetings(prev => prev.filter(m => m._id !== selectedMeeting._id));
        setOpenDetailsDialog(false);
        setSelectedMeeting(null);
      } else {
