...
const Profile = () => {
  ...
  // Get available subjects that haven't been selected yet (for teaching)
  const getAvailableTeachingSubjects = () => {
    const teachingSubjects = editProfile.subjectsToTeach.map(s => s.subject);
    const learningSubjects = editProfile.subjectsToLearn.map(s => s.subject);
    return SUBJECTS.filter(subject => 
      !teachingSubjects.includes(subject) && !learningSubjects.includes(subject)
    );
  };

  // Get available subjects that haven't been selected yet (for learning)
    ...
    <Container maxWidth="lg" sx={{ mt: 4, mb: 4 }}>
      <Paper className="glass" sx={{ 
        ...
        <Grid container spacing={3}>
          ...
          {/* Learning Subjects - Now managed in Matches page */}
          <Grid item xs={12} md={6}>
            <Card className="glass" sx={{
              borderRadius: 4,
              background: 'rgba(255, 255, 255, 0.15)',
            ...
            >
              <CardContent sx={{ textAlign: 'center', py: 4 }}>
                ...
                <Typography variant="body2" color="text.secondary" sx={{ mb: 2 }}>
                  Manage your learning interests directly in the Matches page for better peer discovery and real-time matching.
                </Typography>
                <Button 
                  variant="contained" 
                  color="primary"
                ...
                <Box sx={{ display: 'flex', gap: 1, mb: 2, alignItems: 'center' }}>
                  <FormControl sx={{ flex: 1 }}>
                    ...
                    <Select
                      ...
                      size="small"
                    >
                      {getAvailableTeachingSubjects().map((subject) => (
              ...
              <Grid item xs={12} md={6}>
                ...
                <Box sx={{ 
                  ...
                  <Typography variant="body2" color="text.secondary" gutterBottom>
                    Learning interests are now managed in the Matches page for better match discovery.
                  </Typography>
                  <Button 
                    variant="outlined" 
                    color="primary" 
